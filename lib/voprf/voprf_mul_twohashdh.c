...
static enum voprf_error blind(
  ...
  CURVE_SUCCESS_CHECK(voprf->curve->hash_to_curve(
      ...
      1));
  CURVE_SUCCESS_CHECK(voprf->curve->group_op(
      blinded_element,
      blinded_element_len,
      hashed_message_point,
      element_bytes,
...
static enum voprf_error verifiable_unblind(
  ...
  if (unblinded_element_len != element_bytes ||
      ...
      blinded_element_len != element_bytes || pk_len != element_bytes) {
    return VOPRF_BUFFER_LENGTH_ERROR;
  }
  CURVE_SUCCESS_CHECK(
  ...
  // Unblind evaluation
  unsigned char cancellation_point[element_bytes];
  CURVE_SUCCESS_CHECK(voprf->curve->group_exp(
      ...
      pk,
      pk_len));
  CURVE_SUCCESS_CHECK(voprf->curve->group_inv_op(
      unblinded_element,
